...
class Suppliers extends Person_controller
{
	...
	*/
	...
	function exists_email($email='')
	{
		...
		if($this->Supplier->exists_email($email)!='0')
		{	
			//si existe
			echo json_encode(array('success'=>true,'message'=>'existe', 'email'=>$this->Supplier->exists_email($email)));
		}else{
			//no existe
			echo json_encode(array('success'=>false,'message'=>'no existe','email'=>$this->Supplier->exists_email($email)));
		}
	}
	...
	*/
	...
	{
		if ($supplier_id==-1){
			if($this->Supplier->exists_email($this->input->post('email'))!='0'){	
				//si existe
				die (
					json_encode(
						array(
							'success'=>true,
							'message'=>'existe', 
							'email'=> $this->Supplier->exists_email($this->input->post('email'))
						)
					)
				);
			}else{
		...
		$supplier_data=array(
			...
			'product_supplied'=>$this->input->post('prod_supplied'),
