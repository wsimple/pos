...
class Items extends Secure_area implements iData_controller
{
	...
	*/
	...
	function view($item_id=-1,$action=false){
		$data['item_info']=$this->Item->get_info($item_id);
		$data['item_tax_info']=$this->Item_taxes->get_info($item_id);
		$suppliers = array('' => $this->lang->line('items_none'));
		if ($action){
			switch ($action) {
				case 'receiving': case 'shipping': case 'sales':
				$data['sendto']=$action; break; 
			}
		}
	...
	function save($item_id=false,$action=false){
		if ($item_id==-1 || $item_id=='-1') $item_id=false;
		$item_data = array(
		...
		$id=$this->Item->save($item_data,$item_id);
		if($id){
			if ($item_id!==false){
				$id=$item_id;
			...
			if($item_id===false){
				...
				$item_data['name'],'item_id'=>$id,'pictures'=>$dat,'action'=>$action));
